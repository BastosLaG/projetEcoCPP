...
//Elle tu sais jamais ou l'a mettre
...
class Mouton : public Vivant {
    ...
    int lavidaloca(Mouton* tabu[NBR]){
        ...
        faim--;
        if (faim == 0)
        //gère quans un mouton crève de faim(trop con il a ps compris que le ramadan c'était fini)
...
class Loup : public Vivant {
    ...
    //lavidaquimacasserlescouilles
    int lavidaloca(Loup* tabu[NBR]){
        pv--;
        faim--;
        //gère quans un loup crève de faim(trop con il a ps compris que le ramadan c'était fini)
        if (faim == 0)
        {
...
//On est dans minecraft ouuuuuuuuu ? Genrateur de moutmout et de carnivor
...
void tourMouton(Mouton* moutons[NBR], int _nbr_moutons){
    ...
    for(int a = 0; a < _nbr_moutons; a++) {
        ...
        //gère les différentes actions (même si on préfèere la troisème)
        else{
            if(temp == 0) {
...
//la fonction qui a été le moins touché
void initialisation(){
